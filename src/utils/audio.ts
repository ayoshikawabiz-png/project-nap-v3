...
export function stopAlarm() {
  ...
  // Close alarm context only — tap audio stays available for button feedback
  if (alarmCtx && alarmCtx.state !== 'closed') {
    alarmCtx.close();
  }
  alarmCtx = null;
...
export function unlockAudio() {
  ...
  // Prime HTML audio silently so later taps play without delay
...
export function playTapSound() {
  try {
    ...
    // Context not yet running (common on iOS) — HTML Audio plays within user gesture
    playTapHtml();
    void ctx.resume();
  } catch {
    playTapHtml();
